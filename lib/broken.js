...
var ClassFactory = require('atma-class');
    ...
    Static: {
        create: function (mix, statusCode) {
            if (dependency_1.is_String(mix))
                return exports.HttpError(mix, statusCode);
            if (mix._error != null)
                return mix;
            if (mix instanceof Error) {
                var error = exports.HttpError(mix.message, statusCode || 500);
                error._error = mix;
                return error;
            }
            if (dependency_1.is_Object(mix)) {
                if (mix.toString !== _obj_toString) {
                    return exports.HttpError(mix.toString(), statusCode || mix.statusCode || mix.status);
                }
                var msg = mix.message, code = statusCode || mix.statusCode || mix.status, error;
                error = exports.HttpError(msg, code);
                error._json = mix;
                return error;
            }
            return exports.RuntimeError('Invalid error object: ' + mix);
        }
    }
});
...
exports.cfg_prepair = function (base, configs, defaults) {
...
exports.configurate_Pages = function (cfg) {
    var pages = cfg.pages;
    if (pages == null)
        return;
    var page, key;
...
exports.configurate_PageFiles = function (cfg, app) {
    ...
    return dependency_1.io
        ...
        files.forEach(function (file) {
            ...
            var rule = {
                rule: "^" + name + "/?$ " + dependency_1.Uri.combine(folder, path) + "/index.mask.html"
            };
            console.log(rule.rule);
            if (cfg.rewriteRules == null) {
                cfg.rewriteRules = [];
            }
            cfg.rewriteRules.push(rule);
        });
    });
};
...
var HttpSubApplication = (function (_super) {
    ...
    function HttpSubApplication(path, mix, parentApp) {
        ...
        var configs = mix.configs, config = mix.config;
    ...
    HttpSubApplication.prototype.pipe = function (req, res) {
...
var Rewriter = (function () {
    ...
    Rewriter.prototype.rewrite = function (req) {
        ...
        console.log(req.url, 'AFTER', this.rules);
    };
    return Rewriter;
}());
...
;
					...
					if (isObject(_src_loader_yml) && isObject(module.exports)) {
						Object.assign(_src_loader_yml, module.exports);
						return;
					}
					_src_loader_yml = module.exports;
				}());
				// end:source ./templates/ModuleSimplified.js
...
exports.default = __assign({ Application: Application_1.default,
    ...
    HttpService: HttpService_1.default, middleware: export_1.default, clean: Application_1.default.clean }, vars_1.server);
;
...
const server = _src_export['default'];
