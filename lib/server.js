...
function send_JSON(req, res, json, statusCode, headers) {
    var str;
    try {
        str = JSON.stringify(json);
    }
    catch (error) {
        return send_Error(req, res, new HttpError_1.RuntimeError("Json Serialization: " + error.message));
    }
    send_Content(req, res, str, statusCode || 200, mime_1.mime_JSON, headers);
}
exports.send_JSON = send_JSON;
;
function send_Error(req, res, error, headers) {
    if (error instanceof HttpError_1.HttpError === false) {
        error = HttpError_1.HttpError.create(error);
    }
    send_Content(req, res, JSON.stringify(error), error.statusCode || 500, mime_1.mime_JSON, headers);
}
exports.send_Error = send_Error;
;
function send_Content(req, res, content, statusCode, mimeType, headers) {
    if (typeof content !== 'string' && content instanceof Buffer === false) {
        ...
        send_Error(req, res, new HttpError_1.RuntimeError('Unexpected content response'), headers);
        return;
    }
