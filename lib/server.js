...
var path_1 = _src_util_path;
var dependency_1 = _src_dependency;
var WatcherHandler = (function (_super) {
...
function handler_process(app, handler, req, res) {
    dependency_1.logger(95)
        .log('<request>', req.url);
    var result = handler.process(req, res, app.config);
    if (result != null) {
...
function handler_await(app, handler, req, res, dfr) {
    dfr.then(function onSuccess(content, statusCode, mimeType, headers) {
        ...
        send_1.send_Error(res, error, allHeaders);
    });
}
function handler_complete(app, handler, res, content, statusCode, mimeType, headers) {
