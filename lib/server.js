...
(function () {
	...
	// end:source uri.es6
	// source page.js
	var page_flatternPageViewRoutes;
	(function(){
		page_flatternPageViewRoutes = function (pages, pageCfg) {
			var out = {},
				key, page;
			for(key in pages) {
				page = pages[key];
		...
		function merge(pageData, subPagePath, subPage, prevSegment, currentSegment) {
			...
			for (var key in subPage) {
				if (key === 'view' || key === 'route' || key === 'id') continue;
