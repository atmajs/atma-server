...
var atma_utils_1 = require("atma-utils");
var ruta_1 = require("ruta");
var obj_1 = require("class-json/utils/obj");
var fns_RESPONDERS = [
    'subapps',
    'handlers',
    'services',
...
(function () {
    ...
    memory_resolve = function (factory, url, route, callback) {
        var match = rgx.exec(url), str = match[0], type = match[1], path = url.substring(match.index + match[0].length), memory = getContext(type), Handler = obj_1.obj_getProperty(memory, path);
        if (Handler == null) {
            dependency_1.logger.error('<handler> invalid route', url);
            callback(new ErrorHandler('Invalid route: ' + url));
            return;
