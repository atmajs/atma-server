...
export function send_JSON (req: IncomingMessage, res: ServerResponse, json, statusCode, headers) {
    let str;
    try {
    ...
    send_Content(req, res, str, statusCode || 200, mime_JSON, headers);
};

export function send_Error (req: IncomingMessage, res: ServerResponse, error, headers?) {
...
export function send_Content (req: IncomingMessage, res: ServerResponse, content, statusCode, mimeType, headers?) {
